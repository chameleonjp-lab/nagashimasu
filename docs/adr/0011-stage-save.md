...
- 同じステージを「このステージを始める」で新規開始した場合は、そのステージの途中保存を破棄する。クリアした場合は演出前に進捗保存を試み、成功後に対応する途中保存を破棄する。進捗保存に失敗した間は、既存の途中保存を先に消さない。失敗で終了した場合は受理時点で途中保存を破棄する。
- 進捗保存失敗後にUndoや別のゲームへ進んだ場合、クリア記録の再保存が新しい途中保存を消さないよう、記録の確定と途中保存の片付けを分けて扱う。
