...
確定・見送り・Undoの結果は、`StageSession`が返す`StageTraceEvent`をPresentationの`TracePlayback`へ渡して短く再生する。traceの順序と`FlowStepResult`はそのまま使い、画面側で水流を再計算しない。

- traceは施工、雨、各水流step、評価（UndoはUndoイベント）の順に表示する。
- 再生時間はPresentationの固定値であり、`Date`、`performance.now`、`requestAnimationFrame`をDomainへ渡さない。
- 再生中は盤面、候補、回転、取消、確定、見送り、Undo、再挑戦を無効化する。Pointer Eventも仮置きを変更しない。
- 再生が完了してから、snapshotの`cleared`／`failed`、スコア、失敗理由を結果パネルへ表示する。
- 再挑戦は新しい`StageController`を作るだけで、保存やDomainの状態巻き戻しを行わない。

### 8. 状態説明の読み上げ

- 現在の目的と雨予報は、手番の確定後に更新される文章として`aria-live="polite"`で通知する。
- タイマーの残り秒数は高頻度に変化するため、ライブリージョンにはしない。
- 危険理由、施工プレビュー、結果、操作メッセージは既存の文章領域から通知し、同じ内容をPresentationで別計算しない。

この再生は見せ方の層であり、再生中に手番を追加したり、結果を変更したりしてはならない。
...
- 確定traceの順序と各phaseの再生進捗が決定論的である。
- 再生中の入力が状態を変更せず、終了後だけ結果パネルが表示される。
...
- 再生中に入力を受け付ける、または結果表示前に再挑戦できる。
- `npm run check`、型検査、ドメインsmoke、画面smokeのいずれかが失敗する。

## 追記: ADR 0018との関係

描画方式に関する部分はADR 0018で置き換えたが、Domain境界、入力、仮置き、確定、再生の契約は引き続き有効。
