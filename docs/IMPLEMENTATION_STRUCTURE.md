...
| 表示用変換 | `src/presentation/stage-projection.ts`、`stage-preview.ts`、`stage-copy.ts`、`game-copy.ts`、`candidate-shape.ts`、`candidate-card.ts`、`candidate-card-dom.ts`、`cell-picker-dom.ts`、`mobile-controls-focus.ts`、`mobile-controls-a11y.ts` | 予報、危険度、画面文言、候補カードの形状・基準セル表示、座標ピッカーの表示・アクセシビリティ状態、モバイル操作シートのフォーカス先とアクセシビリティ状態をDomainの結果から作る | 独自の勝敗判定を作る |
...
| 画面進行 | `src/main.ts`、`src/presentation/app-markup.ts` | 画面の表示段階、ボタン、再生の開始・終了をつなぎ、初期画面のHTMLを組み立てる | 水流の規則を直接実装する |
